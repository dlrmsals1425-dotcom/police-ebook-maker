...
4. 실명·사건번호·연락처를 뺀다. 실사례는 `가상 사례 ·` 제목으로 재구성한다.
...
7. 배포는 제작기 **내보내기 → HTML 저장**이다. 결과 HTML은 표지·조판을 유지한 전자책(휴대폰 한 쪽씩, PC 두 쪽 펼침)이어야 한다. 카드 목록처럼 쌓지 않는다.
8. 본문에 「이 교육자료는」처럼 자료 자체를 설명하는 문장을 넣지 않는다. 책은 내용만 말한다.

## 공개 배포

`외부유출금지` 원문과 실사례 재구성 MD/HTML은 공개 GitHub에 올리지 않는다. 로컬 또는 사용자 USB에만 둔다. 공개해도 되는 책만 `books/` 에 넣고 push 한다. 가상 샘플(`samples/가정폭력_현장조치.md`, `samples/전자책_활용가이드.md`, `book.html`)만 저장소에 둔다.
