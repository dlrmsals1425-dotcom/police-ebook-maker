...
(function (g) {
  ...
  function bookCss() {
    return [
      ...
      ".turn-sheet{transform-origin:left center;backface-visibility:hidden;-webkit-backface-visibility:hidden;will-change:transform;box-shadow:12px 0 28px rgba(18,14,10,.24);}",
  ...
  function bookScript(storeKey, startIndex, preview) {
    ...
    return (
      ...
      [
        ...
        "},{passive:false});",
        "st.addEventListener('touchcancel',function(){tracking=false;if(turn)settleTurn(false);},{passive:true});",
        ...
        "if(phone()||Date.now()<ignoreClickUntil||selected())return;",
